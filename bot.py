...
from engine import SYMBOLS, daily_risk_halted, get_signal_snapshot, paper_close, paper_open, paper_records, paper_stats, paper_status, save_event
...
def auth(update):
    return not ALLOWED or (update.effective_user and str(update.effective_user.id) in ALLOWED)
def fmt(v,d=4):
    if v is None:return "—"
...
def main_kb():
    return InlineKeyboardMarkup([[InlineKeyboardButton("📊 داشبورد",callback_data="dashboard"),InlineKeyboardButton("📡 بازار",callback_data="markets")],[InlineKeyboardButton("🟢 سیگنال",callback_data="signal"),InlineKeyboardButton("🧠 AI Review",callback_data="review")],[InlineKeyboardButton("📒 Paper",callback_data="paper"),InlineKeyboardButton("📈 عملکرد",callback_data="stats")],[InlineKeyboardButton("🔄 بروزرسانی",callback_data="refresh"),InlineKeyboardButton("🛡️ ایمنی",callback_data="safety")],[InlineKeyboardButton("⚙️ تنظیمات",callback_data="settings"),InlineKeyboardButton("❓ راهنما",callback_data="help")]])
def symbol_kb():
    return InlineKeyboardMarkup([[InlineKeyboardButton(s,callback_data=f"sig:{s}") for s in SYMBOLS],[InlineKeyboardButton("⬅️ خانه",callback_data="home")]])
def signal_text(s):
    d=s.get("decision","NO TRADE"); label={"LONG":"🟢 LONG","SHORT":"🔴 SHORT","NO TRADE":"🟡 NO TRADE"}.get(d,d)
    a=[f"📊 Trading Guardian | {s.get('symbol','—')}",f"تصمیم: {label}",f"قیمت: {fmt(s.get('price'))}",f"امتیاز: {s.get('score','—')}",f"اطمینان: {s.get('confidence',0)}%"]
    if d!="NO TRADE": a += [f"Entry: {fmt(s.get('entry'))}",f"🛑 SL: {fmt(s.get('stop'))}",f"🎯 TP1: {fmt(s.get('tp1'))} | R:R {s.get('risk_reward_tp1','—')}",f"🎯 TP2: {fmt(s.get('tp2'))} | R:R {s.get('risk_reward_tp2','—')}",f"🎯 TP3: {fmt(s.get('tp3'))} | R:R {s.get('risk_reward_tp3','—')}"]
    der=s.get("derivatives",{}); a += [f"Funding: {fmt(der.get('funding_rate'),6)} | OI: {fmt(der.get('open_interest'),2)}",f"دلیل: {s.get('reason','—')}",f"ابطال: {s.get('invalidation','—')}","⚠️ فقط تحلیل/Paper؛ سفارش واقعی فعال نیست."]
    return "\n".join(a)
def dashboard():
...
async def send_signal(target,symbol):
    try:
        s=await asyncio.to_thread(get_signal_snapshot,symbol); save_event("telegram_signal",s); b=[]
        if s.get("decision") in {"LONG","SHORT"} and not daily_risk_halted():b.append(InlineKeyboardButton("📒 ثبت Paper",callback_data=f"paperopen:{symbol}"))
        b += [InlineKeyboardButton("🔄 دوباره",callback_data=f"sig:{symbol}"),InlineKeyboardButton("🏠 خانه",callback_data="home")]
        await target.reply_text(signal_text(s),reply_markup=InlineKeyboardMarkup([b]))
...
async def callback(update,context):
    ...
    if d=="review":
        ...
        try:
            s=await asyncio.to_thread(get_signal_snapshot); r=await asyncio.to_thread(review_with_ai,client,s); save_event("review",{"snapshot":s,"result":r}); await q.message.reply_text(r[:4000],reply_markup=main_kb())
    ...
    if d.startswith("paperopen:"):
        s=await asyncio.to_thread(get_signal_snapshot,d.split(":",1)[1]); r=await asyncio.to_thread(paper_open,s,1.0); return await q.message.reply_text(("✅ در Paper ثبت شد.\n" if r.get("ok") else "⚠️ ثبت نشد: ")+str(r.get("trade") if r.get("ok") else r.get("reason","—")),reply_markup=main_kb())
