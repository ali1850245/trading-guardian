...
def kb_symbols():
    rows = [[InlineKeyboardButton(s, callback_data=f"sig:{s}") for s in SYMBOLS[:3]]]
    rows.append([InlineKeyboardButton("⬅️ منوی اصلی", callback_data="home")])
...
def paper_text():
    ...
    if active:
        text += "پوزیشن‌های باز:\n" + "\n".join(
            f"• {x['id']} | {x['symbol']} {x['side']} | Entry {fmt(x['entry'])} | SL {fmt(x['stop'])} | TP1 {fmt(x['tp1'])}"
            for x in active[:10]
        )
    else:
        text += "پوزیشن بازی ثبت نشده است."
    return text

def format_signal(s):
    d = s.get("decision", "NO TRADE")
    label = {"LONG": "🟢 LONG", "SHORT": "🔴 SHORT", "NO TRADE": "🟡 NO TRADE"}.get(d, d)
    lines = [f"📊 Trading Guardian | {s.get('symbol', '-')}", f"تصمیم: {label}",
             f"قیمت: {fmt(s.get('price'))}", f"امتیاز: {s.get('score', '—')}",
             f"اطمینان محاسباتی: {s.get('confidence', 0)}%"]
    if d != "NO TRADE":
        lines += [f"Entry: {fmt(s.get('entry'))}", f"🛑 SL: {fmt(s.get('stop'))}",
                  f"🎯 TP1: {fmt(s.get('tp1'))} | R:R {s.get('risk_reward_tp1', '—')}",
                  f"🎯 TP2: {fmt(s.get('tp2'))} | R:R {s.get('risk_reward_tp2', '—')}",
                  f"🎯 TP3: {fmt(s.get('tp3'))} | R:R {s.get('risk_reward_tp3', '—')}" ]
    der = s.get("derivatives", {})
    lines += [f"Funding: {fmt(der.get('funding_rate'), 6)} | OI: {fmt(der.get('open_interest'), 2)}",
              f"دلیل: {s.get('reason', '—')}", f"ابطال: {s.get('invalidation', '—')}",
              "⚠️ خروجی فقط Paper/تحلیلی است؛ سفارش واقعی فعال نیست."]
    return "\n".join(lines)
...
def safety_text():
    p = paper_status()
    return ("🛡️ وضعیت ایمنی\n\nحالت: PAPER ONLY\nسفارش واقعی: خاموش\nبرداشت: متصل نیست\n"
            f"Daily loss limit: {p['daily_loss_limit_pct']}%\n"
            f"Kill Switch: {'🔴 فعال' if p['kill_switch'] else '🟢 آماده'}\n"
...
async def send_signal(target, symbol):
    try:
        ...
        save_event("telegram_signal", s)
        buttons = []
        if s.get("decision") in {"LONG", "SHORT"} and not daily_risk_halted():
            buttons.append(InlineKeyboardButton("📒 ثبت در Paper", callback_data=f"paperopen:{symbol}"))
        ...
        await target.reply_text(f"⚠️ خطا: {e}", reply_markup=kb_main())
...
async def callback(update, context):
    ...
    if data in {"home", "dashboard"}:
        return await q.edit_message_text(dashboard_text(), reply_markup=kb_main())
    if data == "refresh":
        await q.edit_message_text("🔄 در حال دریافت داده تازه...", reply_markup=kb_main())
        return await send_signal(q.message, os.getenv("SYMBOL", SYMBOLS[0]))
    if data == "markets":
        return await q.edit_message_text(markets_text(), reply_markup=kb_symbols())
    if data == "stats":
        return await q.edit_message_text(stats_text(), reply_markup=kb_main())
    if data == "paper":
        return await q.edit_message_text(paper_text(), reply_markup=kb_main())
    if data == "safety":
        return await q.edit_message_text(safety_text(), reply_markup=kb_main())
    if data == "help":
        return await q.edit_message_text(help_text(), reply_markup=kb_main())
    if data == "settings":
        return await q.edit_message_text("⚙️ تنظیمات\n\nنماد پیش‌فرض: " + os.getenv("SYMBOL", "BTCUSDT") +
                                         "\nبازه مانیتور: " + str(MONITOR_INTERVAL) +
                                         " ثانیه\nAuto Paper Exit: " + str(PAPER_AUTO_EXIT) +
                                         "\n\nکلیدها فقط از Environment خوانده می‌شوند.", reply_markup=kb_main())
    if data == "signal":
        return await q.edit_message_text("🟢 نماد را انتخاب کن:", reply_markup=kb_symbols())
    if data == "review":
        if client is None:
            return await q.edit_message_text("⚠️ OPENAI_API_KEY تنظیم نشده است.", reply_markup=kb_main())
        await q.edit_message_text("🧠 در حال بررسی آخرین وضعیت...", reply_markup=kb_main())
        await q.message.chat.send_action(ChatAction.TYPING)
        try:
            s = await asyncio.to_thread(get_signal_snapshot)
            result = await asyncio.to_thread(review_with_ai, client, s)
            save_event("review", {"snapshot": s, "result": result})
            await q.message.reply_text(result[:4000], reply_markup=kb_main())
        except Exception as e:
            await q.message.reply_text(f"⚠️ خطای AI: {e}", reply_markup=kb_main())
        return
    if data.startswith("sig:"):
        await q.message.chat.send_action(ChatAction.TYPING)
        return await send_signal(q.message, data.split(":", 1)[1])
    if data.startswith("paperopen:"):
        symbol = data.split(":", 1)[1]
        s = await asyncio.to_thread(get_signal_snapshot, symbol)
        result = await asyncio.to_thread(paper_open, s, 1.0)
        return await q.message.reply_text(("✅ در Paper ثبت شد.\n" if result.get("ok") else "⚠️ ثبت نشد: ") +
                                          (str(result.get("trade")) if result.get("ok") else result.get("reason", "—")), reply_markup=kb_main())
...
async def stats_cmd(update, context):
    if authorized(update):
        await update.message.reply_text(stats_text(), reply_markup=kb_main())

async def paper_cmd(update, context):
    if authorized(update):
        await update.message.reply_text(paper_text(), reply_markup=kb_main())
                ...
                if PAPER_AUTO_EXIT:
                    for trade in active_paper_positions():
                        ...
                        if price is None:
                            continue
                        side = trade.get("side")
                        stop, tp1 = float(trade.get("stop")), float(trade.get("tp1"))
