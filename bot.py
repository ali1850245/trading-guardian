...
client=OpenAI(api_key=OPENAI_KEY, timeout=30.0) if OPENAI_KEY else None
...
def timeframe_kb(symbol=None):
    rows=[]
    for i in range(0,len(TIMEFRAME_KEYS),3): rows.append([InlineKeyboardButton(tf,callback_data=f"tf:{tf}:{symbol or os.getenv('SYMBOL',SYMBOLS[0])}") for tf in TIMEFRAME_KEYS[i:i+3]])
...
async def get_signal_with_timeout(symbol,timeout=30): return await asyncio.wait_for(get_signal(symbol),timeout=timeout)
async def send_signal(target,symbol,selected_tf=None,leverage=None):
    try:
        s=await get_signal_with_timeout(symbol); save_event("telegram_signal",s); b=[]
        if s.get("decision") in {"LONG","SHORT"} and not daily_risk_halted(): b.append(InlineKeyboardButton("📒 ثبت Paper",callback_data=f"paperopen:{symbol}:{selected_tf or '15m'}:{leverage or 1}"))
        ...
        await target.reply_text(signal_text(s,selected_tf,leverage),reply_markup=InlineKeyboardMarkup([b,b2,[InlineKeyboardButton("🏠 خانه",callback_data="home")]]))
    except asyncio.TimeoutError: await target.reply_text("⚠️ تحلیل بیش از ۳۰ ثانیه طول کشید و متوقف شد. اتصال داده بازار/Wallex را بررسی می‌کنیم.",reply_markup=main_kb())
    except Exception as e: await target.reply_text(f"⚠️ خطا در تحلیل: {type(e).__name__}: {e}",reply_markup=main_kb())
...
async def callback(update,context):
    ...
    try:
        ...
        if d=="review":
            ...
            await q.edit_message_text("🧠 در حال بررسی...",reply_markup=main_kb())
            await q.message.chat.send_action(ChatAction.TYPING)
            try:
                s=await get_signal_with_timeout(os.getenv("SYMBOL",SYMBOLS[0]),30)
                r=await asyncio.wait_for(asyncio.to_thread(review_with_ai,client,s),timeout=35)
                save_event("review",{"snapshot":s,"result":r})
                await q.message.reply_text(r[:4000],reply_markup=main_kb())
            except asyncio.TimeoutError:
                await q.message.reply_text("⚠️ زمان تحلیل تمام شد. داده بازار یا سرویس AI پاسخ نداد.",reply_markup=main_kb())
            except Exception as e:
                await q.message.reply_text(f"⚠️ خطای AI: {type(e).__name__}: {e}",reply_markup=main_kb())
            return
        ...
        if d.startswith("result:"):
            _,symbol,tf=d.split(":",2); s=await get_signal_with_timeout(symbol); return await q.edit_message_text(target_result_text(s),reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📒 Paper",callback_data=f"paperopen:{symbol}:{tf}:1")],[InlineKeyboardButton("🔄 دوباره",callback_data=f"sig:{symbol}"),InlineKeyboardButton("🏠 خانه",callback_data="home")]]))
        if d.startswith("paperopen:"):
            _,symbol,tf,lev=d.split(":",3); s=await get_signal_with_timeout(symbol); r=await asyncio.to_thread(paper_open,s,1.0); save_event("paper_parameters",{"symbol":symbol,"timeframe":tf,"leverage":int(lev),"signal_id":s.get("signal_id"),"paper_result":r}); return await q.message.reply_text(("✅ در Paper ثبت شد.\n" if r.get("ok") else "⚠️ ثبت نشد: ")+str(r.get("trade") if r.get("ok") else r.get("reason","—"))+f"\n⏱️ TF: {tf}\n⚙️ Leverage scenario: {lev}x",reply_markup=main_kb())
    except asyncio.TimeoutError:
        await q.message.reply_text("⚠️ عملیات بیش از حد طول کشید و متوقف شد. دوباره امتحان کن.",reply_markup=main_kb())
    except Exception as e:
        await q.message.reply_text(f"⚠️ خطای دکمه: {type(e).__name__}: {e}",reply_markup=main_kb())
    ...
    while True:
        try:
            for symbol in SYMBOLS:
                s=await get_signal_with_timeout(symbol,30); sig=(symbol,s.get("decision"),s.get("entry"),s.get("stop"),s.get("tp1"),s.get("tp2"),s.get("tp3"))
